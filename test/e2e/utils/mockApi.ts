    ...
    if (path === '/api/ndas/company-defaults' && method === 'GET') {
      return jsonResponse(route, 200, {
        defaults: {
          ...
          typicalAuthorizedPurpose: 'Proposal support',
          authorizedPurposeCounts: [{ purpose: 'Proposal support', count: 3 }],
          typicalNdaType: 'MUTUAL',
          ndaTypeCounts: [{ ndaType: 'MUTUAL', count: 5 }],
          effectiveDateSuggestions: [new Date().toISOString().split('T')[0]],
