...
- **Existing Files:** `prisma/schema.prisma`, `src/server/services/documentService.ts`, `src/server/services/s3Service.ts`, `src/server/services/documentGenerationService.ts`, `src/server/services/templateService.ts`
- **New Files:** `src/server/utils/versionNumberHelper.ts`, `src/server/utils/documentNotesGenerator.ts`

**Findings:**
- Core metadata fields already exist in Prisma and are populated for uploads; generated documents were missing version/notes/size.
- S3 metadata tagging existed but lacked document-type and version-number tags.
- DB constraints and composite indexes would require a migration; deferred.

**Codebase Scan:**
- Upload flow in `documentService.uploadNdaDocument` already sets fileType, fileSize, notes, versionNumber.
- Generated docs in `documentGenerationService` lacked versionNumber and notes.
- S3 upload adds metadata tags via `s3Service.uploadDocument`.

**Status:** Ready for implementation

## Smart Batching Plan

No safe batchable patterns detected (mixed service updates + tests).
