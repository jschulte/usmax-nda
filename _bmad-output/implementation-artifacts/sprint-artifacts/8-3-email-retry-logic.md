...
1. **Given** email send fails
2. **When** failure detected
3. **Then** system queues for retry (pg-boss), Retries with exponential backoff (max 3 attempts)
4. **And** permanently failed emails trigger alerts
...
- Existing signals: Implemented in emailService (pg-boss retry)
